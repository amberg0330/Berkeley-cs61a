...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1): 
        ...
        dif = cur_score - last_score 

        if dif > running_high:
            high_new = dif
        elif dif <= running_high:
            high_new = running_high
        
        if dif > running_high:
            print (dif, "point(s)! The most yet for Player", who)
        return announce_highest(who, cur_score, high_new)
    return say
...
def make_averaged(original_function, trials_count=1000):
    ...
    def ave_func(*args):
        ...
        while i <= trials_count: 
            sum_tot += original_function(*args)
            i += 1
        return sum_tot/trials_count 
    return ave_func
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    score1 = averaged_roll_dice(1, dice)
    roll_big = 1
    for i in range(2, 11): 
        score2 = averaged_roll_dice(i, dice)
        if score2 > score1: 
            roll_big = i 
            score1 = score2 
    return roll_big 
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    add_score = free_bacon(opponent_score)
    if add_score >= cutoff or extra_turn(score + add_score, opponent_score):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
